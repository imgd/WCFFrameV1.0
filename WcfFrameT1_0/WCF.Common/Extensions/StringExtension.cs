...
namespace Extensions.StringExtension
{
    //---------------------------------------------------------------
    //   string 扩展方法类                                                   
    // —————————————————————————————————————————————————             
    ...
    public static class StringExtension
    {
        ...
        {
            string word = "and|exec|select|chr|mid|master|or|truncate|char|declare|join|cmd|;|'|--";//这里加要过滤的SQL字符 
            if (queryString == null)
                return false;

            foreach (string i in word.Split('|'))
            {
                if (queryString.ToLower().IndexOf(i) > -1)
                {
                    return true;
                }
            }
        ...
        public static string DESEncrypt(this string encryptString)
        {
            try
            {
                string encryptKey = ConfigHelper.GetAppSettingsString("base_publicKey");
                byte[] rgbKey = Encoding.UTF8.GetBytes(encryptKey.Substring(0, 8));
                byte[] rgbIV = Keys;
                byte[] inputByteArray = Encoding.UTF8.GetBytes(encryptString);
                DESCryptoServiceProvider dCSP = new DESCryptoServiceProvider();
        ...
        public static string DESDecrypt(this string decryptString)
        {
            try
            {
                string decryptKey = ConfigHelper.GetAppSettingsString("base_publicKey");
