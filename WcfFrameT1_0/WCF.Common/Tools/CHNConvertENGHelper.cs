...
namespace WCF.Common.Tools
{
    ...
    /// 汉字转拼音类
    /// </summary>
    public class CHNConvertENGHelper
    {
        /// <summary>
        /// 拼音编码集合
        /// </summary>
        private Dictionary<int, string> GetCodeCollections()
        {
            #region
            Dictionary<int, string> data = new Dictionary<int, string> { { -20319, "a" }, { -20317, "ai" }, { -20304, "an" }, { -20295, "ang" }, { -20292, "ao" }, { -20283, "ba" }, { -20265, "bai" },   
    ...
    , { -10256, "zun" }, { -10254, "zuo" },{-4923,"xuan"}};
            #endregion
            return data;
        }
        public IEnumerable<char> GetQuickContentsGroupNames()
        {
            #region
        ...
        /// 获取联系人组名称
        /// </summary>
        /// <param name="noteName">联系人名字</param>
        /// <returns></returns>
        public char GetGroupName(string noteName)
        {
            if (noteName == null || noteName.Length <= 0)
                return '#';
            
            char groupName = char.Parse(ChineseToPinYin(noteName).Substring(0, 1).ToUpper());

            if (GetQuickContentsGroupNames().Contains(groupName))
                return groupName;
            else
                return '#';
        }
        ...
        public string ChineseToPinYin(string txt)
        {
            Dictionary<int, string> CodeCollections = GetCodeCollections();
            txt = txt.Trim();
            byte[] arr = new byte[2];
            ...
            {
                arr = System.Text.Encoding.Default.GetBytes(arrChar[j].ToString());
                if (arr.Length == 1)
                {
                ...
                if (charCode > -4923 || charCode < -20319)
                {
        ...
        ///   <param   name="txt"> 需要转换的汉字 </param>   
        ///   <returns> 返回汉字对应的拼音 </returns>   
        public string ChineseToPinYin(string txt, bool q)
        {
            Dictionary<int, string> CodeCollections = GetCodeCollections();
            txt = txt.Trim();
            byte[] arr = new byte[2];
            StringBuilder result = new StringBuilder();
            StringBuilder result_j = new StringBuilder();
            int charCode = 0;
            int arr1 = 0;
            int arr2 = 0;
            char[] arrChar = txt.ToCharArray();
            for (int j = 0; j < arrChar.Length; j++)
            {
                ...
                {
                    result.Append(arrChar[j].ToString());
                    result_j.Append(arrChar[j].ToString().Substring(0, 1));
                    continue;
                }
                arr1 = (short)(arr[0]);
                arr2 = (short)(arr[1]);
                ...
                else
                {
                    if (!CodeCollections.ContainsKey(charCode))
                    {
                        for (int i = charCode; i <= 0; --i)
                        {
                            ...
                            {
                                result.Append("" + CodeCollections[i] + "");
                                result_j.Append(CodeCollections[i].ToString().Substring(0, 1));
                                break;
                            }
                        }
                    }
                    ...
                    {
                        result.Append("" + CodeCollections[charCode] + "");
                        result_j.Append(CodeCollections[charCode].ToString().Substring(0, 1));
                    }
                }
            }

            result.Append("_" + result_j.ToString() + "");
            return result.ToString();
        }
    }
}
