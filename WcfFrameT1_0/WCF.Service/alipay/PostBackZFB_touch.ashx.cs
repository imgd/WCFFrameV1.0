...
namespace WCF.Service.alipay
{
    ...
    /// 2015.04.15
    /// </summary>    
    public class PostBackZFB_touch : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            ZFBReturnPara_Model retmodel = zfbmana.ZFBCallBack(context, true);
            ...
            if (retmodel.iscallbacksuccess)
            {
                ...
                if (retmodel.ispaysuccess)
                {
                    ...
                    try
                    {
                        ...
                        {
                            string.Format("[alipay_zfb_touch] 订单：{0}交易成功,但是确认失败！请联系技术人员调试！ 结果编号：{1}  DateTime:{2}",
                               order_no,
                               result.ToString(),
                    ...
                    {
                        string.Format("[alipay_zfb_touch] 订单：{0}交易成功,但是确认异常！请联系技术人员调试！ 异常信息：{1}  DateTime:{2}",
                               order_no,
                               ex.Message,
                               DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"))
                               .WriteNoteBook(WriteLogUrl, WriteLogPath);
                ...
                {
                    string.Format("[alipay_zfb_touch] 订单：{0}交易或者退款申请失败！  DateTime:{1}",
                               retmodel.order_no,
                               DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"))
                               .WriteNoteBook(WriteLogUrl, WriteLogPath);
                }
            ...
            {
                string.Format("[alipay_zfb_touch] 订单：{0}交易失败！  DateTime:{1}",
                               retmodel.order_no,
                               DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss"))
                               .WriteNoteBook(WriteLogUrl, WriteLogPath);
            }
