...
namespace WxPayAPI
{
    public class WxPayApi
    {
        ...
        */
        ...
        public static WxPayData OrderQuery(WxPayData inputObj,bool isApp, int timeOut = 6)
        {
            string url = "https://api.mch.weixin.qq.com/pay/orderquery";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no") && !inputObj.IsSet("transaction_id"))
            ...
            inputObj.SetValue("appid", WxPayConfig.APPID_APP);//公众账号ID
            inputObj.SetValue("mch_id", WxPayConfig.MCHID_APP);//商户号
            inputObj.SetValue("nonce_str", WxPayApi.GenerateNonceStr());//随机字符串
            inputObj.SetValue("sign", inputObj.MakeSign(true));//签名
        ...
        */
        ...
        public static WxPayData UnifiedOrder(WxPayData inputObj,  out SortedDictionary<string, object> payUrlPara,int timeOut = 6)
        {
            payUrlPara = new SortedDictionary<string, object>();
            string url = "https://api.mch.weixin.qq.com/pay/unifiedorder";
            //检测必填参数
            if (!inputObj.IsSet("out_trade_no"))
            {
            ...
            WxPayData result = new WxPayData();
            payUrlPara = result.FromXml(response);
